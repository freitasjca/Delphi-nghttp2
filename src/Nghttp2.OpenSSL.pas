...
{$ELSE}
  { Delphi (Windows + Linux + macOS): System.SysUtils provides
    SafeLoadLibrary and POSIX shims for GetProcAddress/FreeLibrary. On
    Windows those functions live in Winapi.Windows — pull it in only there.
    SetDllDirectory is Windows-only too (no POSIX equivalent needed —
...
// The library handle type is genuinely different per platform:
//   Windows  → `THandle` (ordinal) — LoadLibrary/GetProcAddress take this
//   POSIX    → `Pointer`           — dlopen/dlsym/dlclose take this
// The types are the SAME SIZE on 64-bit (both 8 bytes), but Delphi Linux
// refuses to convert between them at compile time — E2010 fires for every
// form of cast, reinterpret, or Move-based bit copy tried. The only clean
// solution is a platform-conditional type alias so no cast is ever needed.
//
// `HANDLE_ZERO` gives the platform-appropriate "unloaded" sentinel value
// (`0` on Windows, `nil` on POSIX) — used for the load/free guards.

// TDllHandle: the platform-native DLL handle type per compiler.
//   FPC (any target)     → TLibHandle (from DynLibs — PtrUInt-sized ordinal)
//   Delphi (any target)  → HMODULE (THandle on Win, NativeUInt on POSIX)
// Both are ordinal, pointer-sized, and 0-valued when unloaded. No casts
// needed anywhere in the helpers below — each helper receives arguments in
// exactly the type its underlying API expects.
...
{$ELSEIF DEFINED(MSWINDOWS)}
  ...
  // LPCSTR = PAnsiChar. Symbol names in DLLs are ANSI-encoded regardless
  // of the app's default char width.
  Result := GetProcAddress(ALib, PAnsiChar(AnsiString(AName)));
{$ELSE}
  // Delphi POSIX shim (System.SysUtils): GetProcAddress(HMODULE, PChar)
  // where PChar = PWideChar on modern Delphi. The shim converts to POSIX
  // narrow internally.
...
function ResolveSymbols(ALibSsl, ALibCrypto: TDllHandle): Boolean;

  function GetFrom(AHandle: TDllHandle; const AName: string; out ADest: Pointer): Boolean;
  begin
...
begin
  ...
  if GLibCrypto = HANDLE_ZERO then
  begin
  ...
  begin
    ...
    if GLibSSL <> HANDLE_ZERO then Break;
  end;

  if GLibSSL = HANDLE_ZERO then
  begin
